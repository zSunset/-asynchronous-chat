...
from common.decorat_log import login_required
from common.variables import ACTION, PRESENCE, TIME, USER, \
    ACCOUNT_NAME, RESPONSE, ERROR, DATA, \
    RESPONSE_511, SENDER, MESSAGE_TEXT, MESSAGE, \
    ADD_CONTACT, PUBLIC_KEY, DESTINATION, LIST_INFO, GET_CONTACTS, \
    USERS_REQUEST, PUBLIC_KEY_REQUEST, REMOVE_CONTACT, EXIT, \
...
class MessageProcessor(threading.Thread):
    ...
    def init_socket(self):
        ...
        logger.info(
            f'Запущен сервер, порт для подключений: {self.port} , адрес \
                с которого принимаются подключения: {self.addr}. Если адрес \
                    не указан, принимаются соединения с любых адресов.')
        # Готовим сокет
            ...
            try:
                ...
                logger.info(
                    f'Отправлено сообщение пользователю \
                        {message[DESTINATION]} от пользователя \
                            {message[SENDER]}.')
            except OSError:
    ...
    def process_client_message(self, message, client):
        ...
        if ACTION in message and message[ACTION] == \
                PRESENCE and TIME in message and USER in message:
